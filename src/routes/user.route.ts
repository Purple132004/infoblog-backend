...
const userRoute = new Hono<AppContext>().basePath('users');
...
userRoute.get('/', authMiddleware(), zValidator('query', listSchema), async (c) => {
    try {
...
userRoute.post('/', zValidator('json', createInsertSchema(user)), async (c) => {
    try {

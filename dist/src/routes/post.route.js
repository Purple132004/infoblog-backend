...
const postRoute = new Hono().basePath("posts");
const listSchema = querySchema(z
    .object({
...
postRoute.get("/", zValidator("query", listSchema), async (c) => {
    try {
        ...
        const [{ count: countPost }] = await db
            ...
            .where(and(userId ? eq(posts.userId, userId) : undefined, category ? eq(categories.slug, category) : undefined, search ? ilike(posts.title, `%${search}%`) : undefined));
...
const findSchema = querySchema(z
    .object({
    with: withSchema(relations, "posts"),
})
    .strict());
...
postRoute.post("/", authMiddleware(), zValidator("json", createInsertSchema(posts).omit({ featuredImage: true }).extend({
    categoryIds: z.string().array().optional(),
})), zValidator("query", findSchema), async (c) => {
    try {
...
postRoute.patch("/:id", authMiddleware(), zValidator("json", createUpdateSchema(posts).omit({ featuredImage: true }).extend({
    categoryIds: z.string().array().optional(),
})), zValidator("query", findSchema), async (c) => {
    try {
        const { id } = c.req.param();
